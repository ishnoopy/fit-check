...
export async function createSettingService(payload: Omit<ISetting, "userId">, userId: string) {
    // Check if setting already exists for this user
    const existingSetting = await settingRepository.findByUserId(userId);

    if (existingSetting) {
        throw new BadRequestError("Setting already exists for this user. Use update instead.");
    }

    const setting = await settingRepository.createSetting({ ...payload, userId: userId });
    return setting;
}

export async function updateSettingService(payload: Partial<Omit<ISetting, "userId">>, userId: string) {
    const existingSetting = await settingRepository.findByUserId(userId);
    ...
    return await settingRepository.updateSetting(existingSetting.id as string, { ...payload, userId: userId });
}

export async function upsertSettingService(payload: Partial<Omit<ISetting, "userId">>, userId: string) {
    return await settingRepository.updateSettingByUserId(userId, { ...payload, userId: userId });
}
