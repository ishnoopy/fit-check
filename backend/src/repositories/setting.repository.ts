...
export async function updateSetting(id: string, setting: Partial<ISetting>) {
    const payload = toSnakeCase(setting);
    const doc = await SettingModel.findByIdAndUpdate(id, payload, { new: true, lean: true }).lean();
    return doc ? toCamelCase(doc) as ISetting : null;
}

export async function updateSettingByUserId(userId: string, setting: Partial<ISetting>) {
    const payload = toSnakeCase(setting);
